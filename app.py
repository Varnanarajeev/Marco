...
def vector_embedding():
    if "vectors" not in st.session_state:
        ...
        csv_directory = r"C:\talrop_marco\data"  # CSV directory
        ...
        for csv_file in csv_files:
            ...
            for index, row in df.iterrows():
                # Create detailed content for the document
                content = f"Name: {row['Product_Name']}\nDescription: {row['Description']}\nPrice: {row['Price']}\nStock: {row['Stock_Quantity']}\nSupplier: {row['Supplier_Information']}"
                st.session_state.docs.append(Document(page_content=content, metadata={"source": csv_file}))

        #make sure that  documents are loaded
        if not st.session_state.docs:
            st.error("No documents loaded. Please check the CSV directory.")
            return

        # Spliting documents for better retrieval
